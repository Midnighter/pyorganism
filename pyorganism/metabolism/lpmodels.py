...
def _grb__status(self):
    ...
    if status == self._grb.GRB.LOADED:
        raise PyOrganismError("optimize before accessing flux information", errorno=status)
    elif status == self._grb.GRB.OPTIMAL:
        pass
    elif status == self._grb.GRB.INFEASIBLE:
        raise PyOrganismError("model is infeasible", errorno=status)
    elif status == self._grb.GRB.INF_OR_UNBD:
        raise PyOrganismError("model is infeasible or unbounded", errorno=status)
    elif status == self._grb.GRB.UNBOUNDED:
        raise PyOrganismError("model is unbounded", errorno=status)
    elif status == self._grb.GRB.CUTOFF:
        raise PyOrganismError("model solution is worse than provided cut-off", errorno=status)
    elif status == self._grb.GRB.ITERATION_LIMIT:
        raise PyOrganismError("iteration limit exceeded", errorno=status)
    elif status == self._grb.GRB.NODE_LIMIT:
        raise PyOrganismError("node limit exceeded", errorno=status)
    elif status == self._grb.GRB.TIME_LIMIT:
        raise PyOrganismError("time limit exceeded", errorno=status)
    elif status == self._grb.GRB.SOLUTION_LIMIT:
        raise PyOrganismError("solution limit reached", errorno=status)
    elif status == self._grb.GRB.INTERRUPTED:
        raise PyOrganismError("optimization process was interrupted", errorno=status)
    elif status == self._grb.GRB.SUBOPTIMAL:
        raise PyOrganismError("solution is suboptimal", errorno=status)
    elif status == self._grb.GRB.NUMERIC:
        raise PyOrganismError("optimization aborted due to numeric difficulties", errorno=status)
